...
install_dependencies() {
    ...
    echo "In use $package_manager Install dependencies..."
    if [ "$package_manager" == "apt" ]; then
        apt update && apt install -y "${debian_packages[@]}"
        # Disable Debian's pip protection mechanism (administrator confirmation of risks required).
    ...
    # Install additional Python dependencies globally (forced mode)
